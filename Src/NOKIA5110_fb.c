...
void print_figura_invertido_seguro( pontos_t *p, const figura_t *figura)
{
   if( xSemaphoreTake( tela, 3000 ) == pdTRUE )
	{
		desenha_fig_invertido(p, figura);
		xSemaphoreGive( tela);
	 }
}
...
void deleta_fig_invertido( pontos_t *p,		// [p.x1 p.y1] para o ponto P1 de incersao da figura
											// Caso se deseje alterar a largura e altura da figura
											//    p.x2  e  p.y2 devem ser diferentes de zero, determinando as novas medidas
											// Caso se deseje imprimir a partir de um ponto no meio da figura (se não empregados devem ser zero)
											//    p.x3 e p.y3 determinam esse ponto
...
void desenha_fig_invertido( pontos_t *p,		// [p.x1 p.y1] para o ponto P1 de incersao da figura
											// Caso se deseje alterar a largura e altura da figura
											//    p.x2  e  p.y2 devem ser diferentes de zero, determinando as novas medidas
											// Caso se deseje imprimir a partir de um ponto no meio da figura (se não empregados devem ser zero)
											//    p.x3 e p.y3 determinam esse ponto
